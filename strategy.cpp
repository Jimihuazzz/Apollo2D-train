...
const std::string Strategy::GET_GOAL_CONF = "get_goal.conf";
...
Strategy::read( const std::string & formation_dir )
{
    ...
    // after we get a goal
    M_AfterGoal_formation = readFormation( configpath + GET_GOAL_CONF);
    if ( ! M_AfterGoal_formation)
    {
        std::cerr << "Failed to read After_goal formation" << std::endl;
        return false;
    }
